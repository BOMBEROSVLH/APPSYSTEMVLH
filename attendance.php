<?php
	if(isset($_POST['employee'])){
		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				$sql = "SELECT * FROM attendance WHERE employee_id = '$id' AND date = '$date_now' and time_out = '00:00:00' order by time_in desc limit 1 ";
				...
				$sql = "SELECT *, attendance.id AS uid FROM attendance LEFT JOIN employees ON employees.id=attendance.employee_id WHERE attendance.employee_id = '$id' AND date = '$date_now' order by time_in desc limit 1";
				$query = $conn->query($sql);
				if($query->num_rows < 1){
					$output['error'] = true;
